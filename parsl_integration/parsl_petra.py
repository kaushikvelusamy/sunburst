...
get_images  = sorted(glob.glob(input_directory +"*.tif"))             # genarates list of .tifs in import_directory
...
@python_app
def aps_integration(input_directory, output_directory, img_start_index, img_end_index):
    import os
    import sys
    import numpy as np
    import pyFAI
    ...
    for images_num in tqdm(image_list[int(img_start_index):int(img_start_index)]):
        one_image       = image_list[images_num]                # returns each filename.tif
        input_tiff_file = fabio.open(one_image)                   # opens .tif file

        image_ind       = str(one_image[-9:-4])                         # returns the sequence number of the file
        output_xy_file  = output_directory + "_" + image_ind + ".xy"    # where integrated data will be stored
...
img_start_index =   0
chunk           =   len(image_list) / num_workers
img_end_index   =   chunk

tasks = []
for i in range(num_workers):
    tasks.append(aps_integration(input_directory, output_directory, img_start_index, img_end_index ) )
